...
class ShowPaySlipWidget(QWidget):
    def __init__(self,
        ...
        # self.setWindowTitle("Salary Result")
    ...
    def setupUI(self):
        ...
        self.bttnSave = QPushButton("Save")
    ...
    def save(self):
        filename = QFileDialog.getSaveFileName(self, "Save as", "/", "PDF File(*.pdf)")
        if filename[0] != '':
            generatePdf(filename[0], self.id.text(), self.name.text(), self.designation.text(),
            ...
            msg = QMessageBox(QMessageBox.NoIcon, "Success", "File saved successfully", parent=self)
            msg.exec_()
            self.goBack()